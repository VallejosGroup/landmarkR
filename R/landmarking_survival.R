...
#'
#' @examples
setGeneric("fit_survival_v2", function(x, landmarks, horizons, method, dynamic_covariates = c()) standardGeneric("fit_survival_v2"))
...
#'
#' @examples
setMethod("fit_survival_v2", "Landmarking", function(x, landmarks, horizons, method, dynamic_covariates = c()) {
  ...
  if (length(landmarks) == 1) { # Base case for recursion
    ...
    # Call to method that performs survival analysis
    x@survival_fits[[as.character(landmarks)]] <- method(survival_formula, data = dataset)
  } else {
    # Recursion
    x <- fit_survival_v2(x, landmarks[1], horizons[1], method, dynamic_covariates)
    x <- fit_survival_v2(x, landmarks[-1], horizons[-1], method, dynamic_covariates)
  }
  x
})
